...
while len(pq) > 0:
    ...
    for i in range(len(ActionPre)):
        ...
        if CheckCondition(vertices[x], ActionPre[i]):
            ...
            next_state = ComputeNextState(vertices[x], ActionEff[i])
            ...
            if not CheckVisited(next_state, vertices):
                ...
                # A* Implementation
                # heuristic_cost = Heuristic(next_state, GoalIndicesOneStep, GoalIndicesTwoStep)
                # heapq.heappush(pq, (cost2come[-1] + heuristic_cost, len(vertices) - 1))

                # Dijsktras
                heapq.heappush(pq, (cost2come[-1], len(vertices) - 1))
